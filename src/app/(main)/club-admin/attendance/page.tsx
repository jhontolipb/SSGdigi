...
interface AttendanceSummary {
    ...
    totalExpectedClubMembers: number;
...
export default function ClubAttendancePage() {
  ...
  const [clubSpecificEvents, setClubSpecificEvents] = useState<Event[]>([]);
  ...
  useEffect(() => {
    if (clubAdminClubId) {
      const filteredEvents = allEvents.filter(e => e.organizerType === 'club' && e.organizerId === clubAdminClubId);
      setClubSpecificEvents(filteredEvents);
  ...
  }, [allEvents, clubAdminClubId, selectedEventId]);
  ...
  useEffect(() => {
    if (selectedEventId) {
      ...
      // Accessing setCurrentEventAttendance from context might not be ideal if it's meant to be global.
      // Instead, ensure fetchAttendanceRecordsForEvent handles clearing or setting empty for its own state.
      // For now, assuming fetchAttendanceRecordsForEvent updates currentEventAttendance correctly.
      // If not, currentEventAttendance won't clear automatically here.
      // setCurrentEventAttendance([]); // Potentially remove if context handles it
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedEventId]);

  useEffect(() => {
    if (selectedEventId && currentEventAttendance && clubAdminClubId) {
      ...
      const clubMembers = allUsers.filter(u => u.role === 'student' && u.clubID === clubAdminClubId);
      const totalExpectedClubMembers = clubMembers.length;
      const totalParticipants = new Set(currentEventAttendance.map(r => r.studentUserID)).size;

      const attendanceRateValue = totalExpectedClubMembers > 0 ? (presentCount / totalExpectedClubMembers) * 100 : 0;
      ...
      setSummary({ 
        ...
        late: lateCount, 
        totalParticipants,
        totalExpectedClubMembers, 
        attendanceRate: attendanceRateValue 
      });
    } else {
      setSummary(null);
    }
  }, [currentEventAttendance, allUsers, selectedEventId, clubAdminClubId]);

  const selectedEvent = clubSpecificEvents.find(e => e.id === selectedEventId);
  ...
  if (authLoading && !clubAdminClubId) {
    return <div className="flex justify-center items-center py-10"><Loader2 className="h-8 w-8 animate-spin text-primary" /><p className="ml-2">Loading club data...</p></div>;
  }

  if (!clubAdminClubId) {
    return (
      <Card className="shadow-lg">
        <CardHeader><CardTitle>Club Not Assigned</CardTitle></CardHeader>
        <CardContent><p className="text-muted-foreground">You are not assigned to a club. Attendance monitoring is unavailable.</p></CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Track attendance for your club's events. Data from Firestore.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col md:flex-row gap-4 mb-6 items-center">
            <Select value={selectedEventId} onValueChange={setSelectedEventId} disabled={clubSpecificEvents.length === 0 || authLoading}>
              <SelectTrigger className="md:w-[300px]">
                <SelectValue placeholder={clubSpecificEvents.length === 0 ? "No club events found" : "Select a Club Event"} />
              </SelectTrigger>
              <SelectContent>
                {clubSpecificEvents.length === 0 && <SelectItem value="no-events" disabled>No events for your club</SelectItem>}
                {clubSpecificEvents.map(event => (
          ...
          {!isDataLoading && selectedEvent && summary && (
            <Card className="mb-6 bg-muted/30">
              ...
              <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div><Users className="mx-auto h-6 w-6 text-primary mb-1"/><p className="text-2xl font-bold">{summary.totalExpectedClubMembers}</p><p className="text-sm text-muted-foreground">Total Club Members</p></div>
                ...
                <div><Percent className="mx-auto h-6 w-6 text-blue-500 mb-1"/><p className="text-2xl font-bold">{summary.attendanceRate.toFixed(1)}%</p><p className="text-sm text-muted-foreground">Attendance Rate (Club)</p></div>
              </CardContent>
            </Card>
          )}
                  ...
                  {filteredAttendanceRecords.length > 0 ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={6} className="text-center text-muted-foreground h-24">
                        {clubSpecificEvents.length === 0 && !authLoading ? "No events found for your club." : 
                         !selectedEventId && !authLoading ? "Please select one of your club's events." : 
