...
export default function DepartmentEventsPage() {
  ...
  // OICs specific to this department or unassigned
  const availableOICs = allUsers.filter(u => u.role === 'oic' && (u.departmentID === deptAdminDeptId || !u.departmentID));

  useEffect(() => {
    if (deptAdminDeptId) {
      setDepartmentEvents(allEvents.filter(e => e.organizerType === 'department' && e.organizerId === deptAdminDeptId));
    }
  }, [allEvents, deptAdminDeptId]);
  ...
  const handleSubmit = async () => {
    if (!formData.name || !formData.date || !formData.timeIn || !formData.timeOut || !deptAdminDeptId) {
  ...
  const filteredDepartmentEvents = departmentEvents.filter(event =>
    event.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (event.description && event.description.toLowerCase().includes(searchTerm.toLowerCase()))
  );

  if (!deptAdminDeptId && !authLoading) {
    return <Card><CardHeader><CardTitle>Error</CardTitle></CardHeader><CardContent><p>Not authorized or department not found.</p></CardContent></Card>;
  }
    ...
    <div className="space-y-6">
        ...
        <CardHeader>
           <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              ...
              <CardDescription>Create and manage events for your department (Data from Firestore).</CardDescription>
            </div>
        ...
        <CardContent>
          {authLoading && (
            <div className="flex justify-center items-center py-10">
              ...
              <p className="ml-2">Loading department events...</p>
            </div>
          )}
          {!authLoading && (
            <>
              ...
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {filteredDepartmentEvents.length > 0 ? filteredDepartmentEvents.map((event) => (
                      <TableRow key={event.id}>
                       ...
                       <TableRow>
                        <TableCell colSpan={5} className="text-center text-muted-foreground h-24">
                          No events found for this department{searchTerm ? ' matching your search' : ''}.
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
      ...
      <Dialog open={isFormOpen} onOpenChange={(open) => {if(!isSubmitting) setIsFormOpen(open)}}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingEvent ? 'Edit Department Event' : 'Create New Department Event'}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-2">
            ...
            <div>
                <Label>Assign OICs (Department Specific)</Label>
                <Card className="mt-1 p-3 max-h-40 overflow-y-auto border">
                    ...
                    {availableOICs.length > 0 ? availableOICs.map(oic => (
                        <div key={oic.userID} className="flex items-center space-x-2">
                            <Checkbox
                                id={`oic-dept-${oic.userID}`}
                                checked={formData.oicIds?.includes(oic.userID) || false}
                                onCheckedChange={() => handleOICChange(oic.userID)}
                                disabled={isSubmitting}
                            />
                            <Label htmlFor={`oic-dept-${oic.userID}`} className="font-normal">{oic.fullName}</Label>
                        </div>
                    )) : <p className="text-sm text-muted-foreground">No OICs available for assignment to this department.</p>}
                    </div>
                </Card>
            </div>
